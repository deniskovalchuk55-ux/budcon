...
import {
  ...
  addStaff, updateStaffRates, writeLog,
} from './notion.js'

// ════════════════════════════════════════════════════════
// КОЛЬОРОВА СХЕМА — чорно-біло-сіра, blueprint-стиль
// ════════════════════════════════════════════════════════
const C = {
  bg:       '#000000',
  surface:  '#161618',
...
const F_DISPLAY = "'Unbounded', sans-serif"
...
const inp = {
  ...
  borderRadius: 4, padding: '12px 14px', fontSize: 14, color: C.text,
  fontFamily: F_BODY, boxSizing: 'border-box',
}

const primaryBtn = { background: C.text, color: '#000', border: 'none', borderRadius: 4, padding: '13px 12px', fontSize: 12, fontWeight: 800, cursor: 'pointer', fontFamily: F_BODY, textTransform: 'uppercase', letterSpacing: '0.06em' }
const secondaryBtn = { background: 'transparent', color: C.text, border: `1px solid ${C.border}`, borderRadius: 4, padding: '13px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: F_BODY, textTransform: 'uppercase', letterSpacing: '0.06em' }
const workerCardStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 14, background: C.surface, border: `1px solid ${C.border2}`, borderRadius: 4, cursor: 'pointer' }

// Кутові засічки — сигнатурний елемент, креслярська рамка
function CornerFrame({ children, style = {} }) {
  const tickColor = 'rgba(255,255,255,0.55)'
  const tick = (pos) => {
    const base = { position: 'absolute', width: 9, height: 9, borderColor: tickColor }
    const sides = {
...
const Card = ({ children, top, style = {} }) => (
  <div style={{
    ...
    borderRadius: 4, padding: 14, ...style,
  }}>{children}</div>
)

const Lbl = ({ children, style = {} }) => (
  <div style={{ fontSize: 10, color: C.muted, letterSpacing: '0.18em', marginBottom: 6, fontWeight: 700, textTransform: 'uppercase', fontFamily: F_BODY, ...style }}>{children}</div>
)
...
const Row = ({ label, value, color = C.text, bold = false, last = false }) => (
  <div style={{
    ...
    <span style={{ fontSize: 13, color: C.dim, fontFamily: F_BODY }}>{label}</span>
    <span style={{ fontSize: bold ? 16 : 13, fontWeight: bold ? 800 : 600, color, fontFamily: F_MONO, letterSpacing: '-0.02em' }}>{value}</span>
  </div>
)
...
function Toggle({ on, onClick, label }) {
  return (
    <button onClick={onClick} style={{
      ...
      padding: '12px 14px', borderRadius: 4, cursor: 'pointer', fontFamily: F_BODY,
...
function Sheet({ onClose, title, children }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: '#0a0a0b', border: `1px solid ${C.border}`, borderBottom: 'none', borderRadius: '6px 6px 0 0', width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto', padding: '18px 18px 28px', boxSizing: 'border-box' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: C.text, fontFamily: F_DISPLAY, letterSpacing: '-0.01em' }}>{title}</div>
          <button onClick={onClose} style={{ background: 'transparent', border: `1px solid ${C.border2}`, borderRadius: 3, width: 30, height: 30, color: C.dim, fontSize: 18, cursor: 'pointer', lineHeight: 1 }}>×</button>
        </div>
        {children}
      </div>
    </div>
    ...
    <div style={{ display: 'flex', gap: 4 }}>
      {LANGS.map(l => (
        <button key={l.code} onClick={() => setLang(l.code)} style={{
          padding: '5px 8px', borderRadius: 3, fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: F_MONO, letterSpacing: '0.05em',
...
function Header({ title, sub, onBack, lang, setLang }) {
  return (
    <div style={{ position: 'sticky', top: 0, zIndex: 10, background: C.bg }}>
      <div style={{ borderBottom: `1px solid ${C.border2}`, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        {onBack && (
          <button onClick={onBack} style={{ background: 'transparent', border: `1px solid ${C.border2}`, borderRadius: 3, width: 34, height: 34, color: C.text, fontSize: 16, cursor: 'pointer', flexShrink: 0, fontFamily: F_BODY }}>←</button>
        )}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: C.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: F_DISPLAY, letterSpacing: '-0.01em' }}>{title}</div>
          {sub && <div style={{ fontSize: 10, color: C.dim, marginTop: 3, fontFamily: F_MONO, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{sub}</div>}
        </div>
        {lang !== undefined && setLang && <LangSwitcher lang={lang} setLang={setLang} />}
      </div>
      {/* лінійка-засічки — сигнатурний елемент */}
      <div style={{ height: 5, background: 'repeating-linear-gradient(90deg, rgba(255,255,255,0.14) 0 1px, transparent 1px 12px)' }} />
    </div>
  )
}
...
function MonthPicker({ year, month, onChange, lang }) {
  ...
  const navBtn = { background: 'transparent', border: `1px solid ${C.border2}`, borderRadius: 3, width: 32, height: 32, color: C.text, fontSize: 16, cursor: 'pointer', fontFamily: F_BODY }
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: C.surface, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '10px 12px' }}>
      <button onClick={prev} style={navBtn}>‹</button>
      <div style={{ fontSize: 13, fontWeight: 700, color: C.text, fontFamily: F_MONO, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{months[month]} {year}</div>
...
function LoadingScreen({ lang }) {
  return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.dim, fontSize: 12, fontFamily: F_MONO, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
      {t('loading', lang)}
    </div>
  )
}
...
function NotRegisteredScreen({ lang, setLang, tgId }) {
  return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: F_BODY }}>
      ...
      <div style={{ color: C.muted, fontSize: 11, marginTop: 16, fontFamily: F_MONO, letterSpacing: '0.1em' }}>ID: {tgId}</div>
    </div>
  )
}
  ...
  if (success && result) return (
    <Sheet onClose={onClose} title={t('shiftSaved', lang)}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ background: 'rgba(255,255,255,0.05)', border: `1px solid ${C.border}`, borderRadius: 4, padding: 16 }}>
      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            {[['hours', t('typeHours', lang)], ['m2', t('typeM2', lang)], ['mixed', t('typeMixed', lang)]].map(([k, l]) => (
              <button key={k} type="button" onClick={() => setType(k)} style={{
                padding: '10px 4px', borderRadius: 3, fontSize: 11, fontFamily: F_BODY, cursor: 'pointer',
        ...
        {type !== 'm2' && (
          <div>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {[['range', t('modeRange', lang)], ['manual', t('modeManual', lang)]].map(([k, l]) => (
                  <button key={k} type="button" onClick={() => setHoursMode(k)} style={{
                    padding: '4px 12px', borderRadius: 4, fontSize: 11, fontFamily: F_BODY, cursor: 'pointer',
            ...
            {rawH > 0 && (
              <div style={{ fontSize: 11, color: C.text, marginTop: 6, padding: '6px 10px', background: 'rgba(255,255,255,0.05)', borderRadius: 3, fontFamily: F_MONO }}>
                {hoursMode === 'range'
        ...
        {type !== 'hours' && (
          <div><Lbl>{t('m2', lang)}</Lbl>
            ...
            {m2 && (sel?.rateM2 || 0) > 0 && <div style={{ fontSize: 11, color: C.text, marginTop: 4, fontFamily: F_MONO }}>= {fmt(parseFloat(m2) * (sel?.rateM2 || 0))} {t('uah', lang)}</div>}
          </div>
        )}

        {earn > 0 && (
          <div style={{ background: 'rgba(255,255,255,0.05)', border: `1px solid ${C.border}`, borderRadius: 4, padding: '12px 16px', display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 12, color: C.dim }}>{t('perShift', lang)}</span>
            <span style={{ fontSize: 20, fontWeight: 800, color: C.green, fontFamily: F_MONO, letterSpacing: '-0.02em' }}>{fmt(earn)} {t('uah', lang)}</span>
          </div>
        )}

        {error && <div style={{ color: C.red, fontSize: 12, padding: '8px 12px', background: 'rgba(248,113,113,0.1)', borderRadius: 3, }}>⚠ {error}</div>}
        <button type="submit" disabled={loading} style={{ background: C.text, color: '#000', border: 'none', borderRadius: 4, padding: 14, fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: F_BODY, textTransform: 'uppercase', letterSpacing: '0.06em', opacity: loading ? 0.7 : 1 }}>
...
function AdvanceForm({ worker, allWorkers, isOwner, onClose, onSaved, tgId, tgName, lang }) {
  ...
  if (success) return (
    <Sheet onClose={onClose} title={t('advanceSaved', lang)}>
      <div style={{ textAlign: 'center', padding: 20, fontSize: 26, fontWeight: 800, color: C.green, fontFamily: F_MONO, letterSpacing: '-0.02em' }}>{fmt(amount)} {t('uah', lang)}</div>
    </Sheet>
  )
  ...
  return (
    <Sheet onClose={onClose} title={t('newAdvance', lang)}>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <button type="submit" disabled={loading} style={{ background: C.text, color: '#000', border: 'none', borderRadius: 4, padding: 14, fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: F_BODY, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
...
function DebtForm({ worker, mode, onClose, onSaved, tgId, lang }) {
  ...
  if (success) return (
    <Sheet onClose={onClose} title={mode === 'issue' ? t('debtSaved', lang) : t('debtPaymentSaved', lang)}>
      <div style={{ textAlign: 'center', padding: 20, fontSize: 26, fontWeight: 800, color: mode === 'issue' ? C.red : C.green, fontFamily: F_MONO, letterSpacing: '-0.02em' }}>{mode === 'issue' ? '+' : '−'}{fmt(amount)} {t('uah', lang)}</div>
    </Sheet>
  )
  ...
  return (
    <Sheet onClose={onClose} title={`${t('ratesTitle', lang)} — ${worker.name}`}>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {error && <div style={{ color: C.red, fontSize: 12 }}>{error}</div>}
        <button type="submit" disabled={loading} style={{ background: success ? C.green : C.text, color: '#000', border: 'none', borderRadius: 4, padding: 14, fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: F_BODY, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
...
function AddStaffForm({ onClose, onSaved, lang }) {
  ...
  if (success) return (
    <Sheet onClose={onClose} title={t('staffSaved', lang)}>
      <div style={{ textAlign: 'center', padding: 20, fontSize: 18, fontWeight: 800, color: C.green, fontFamily: F_DISPLAY }}>{name}</div>
    </Sheet>
  )
    ...
    <Sheet onClose={onClose} title={`${t('edit', lang)} — ${date}`}>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button type="button" onClick={del} disabled={loading} style={{ flex: 1, background: 'transparent', border: `1px solid ${C.red}`, color: C.red, borderRadius: 4, padding: 14, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: F_BODY, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t('delete', lang)}</button>
          <button type="submit" disabled={loading} style={{ flex: 2, background: C.text, color: '#000', border: 'none', borderRadius: 4, padding: 14, fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: F_BODY, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{loading ? t('saving', lang) : t('save', lang)}</button>
        </div>
      </form>
    </Sheet>
  )
...
function DaysExpand({ days, rateHour, rateM2, onEdit, lang }) {
  if (!days?.length) return <div style={{ textAlign: 'center', color: C.muted, fontSize: 13, padding: 16, fontFamily: F_BODY }}>{t('noEntries', lang)}</div>
  return (
      ...
      {days.slice().reverse().map(d => {
        ...
        return (
          <div key={d.id} onClick={() => onEdit?.(d)} style={{
            ...
            padding: '8px 10px', borderRadius: 3, background: C.surface2, cursor: onEdit ? 'pointer' : 'default',
          }}>
            <div style={{ fontSize: 12, color: C.dim, fontFamily: F_MONO, letterSpacing: '0.04em' }}>{d.dateShort.split('-').reverse().join('.')}</div>
            <div style={{ display: 'flex', gap: 10, fontSize: 12, fontFamily: F_MONO }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <CornerFrame style={{
        ...
        <div style={{ fontSize: 10, color: C.dim, letterSpacing: '0.28em', fontWeight: 700, textTransform: 'uppercase', fontFamily: F_BODY, marginBottom: 8 }}>
          {t('toPay', lang)} · {w.name}
        </div>
        <div style={{ fontFamily: F_MONO, fontWeight: 800, fontSize: 38, color: C.green, letterSpacing: '-0.03em', lineHeight: 1 }}>
          {fmt(w.final)}<span style={{ fontSize: 16, color: C.dim, fontWeight: 600, marginLeft: 8 }}>{t('uah', lang)}</span>
        </div>
        {w.debtRemaining > 0 && (
          <div style={{ marginTop: 12, fontSize: 11, color: C.red, fontFamily: F_MONO, letterSpacing: '0.05em' }}>
      ...
      <Card>
        ...
        {w.debtPaidMonth > 0 && <Row label={t('debtPaid', lang)} value={`−${fmt(w.debtPaidMonth)} ${t('uah', lang)}`} color={C.red} last />}
      </Card>
    </div>
  )
}
...
function WorkerView({ data, tgId, tgName, year, month, onMonthChange, onRefresh, lang, setLang }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, paddingBottom: 40, fontFamily: F_BODY }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, paddingBottom: 40, fontFamily: F_BODY }}>
      ...
      <div style={{ padding: '16px 16px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <CornerFrame style={{ background: C.surface, border: `1px solid ${C.border2}`, borderRadius: 4, padding: '20px 16px', textAlign: 'center' }}>
          <div style={{ fontSize: 10, color: C.dim, letterSpacing: '0.28em', fontWeight: 700, textTransform: 'uppercase', fontFamily: F_BODY, marginBottom: 8 }}>
            {t('totalToPay', lang)}
          </div>
          <div style={{ fontFamily: F_MONO, fontWeight: 800, fontSize: 38, color: C.green, letterSpacing: '-0.03em', lineHeight: 1 }}>
            {fmt(totalFinal)}<span style={{ fontSize: 16, color: C.dim, fontWeight: 600, marginLeft: 8 }}>{t('uah', lang)}</span>
          </div>
        </CornerFrame>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {allWorkers.map(w => (
              <div key={w.tgId} onClick={() => { setSelWorkerId(w.tgId); setView('worker') }} style={workerCardStyle}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.text, fontFamily: F_BODY }}>{w.name}</div>
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 2, fontFamily: F_MONO, letterSpacing: '0.04em' }}>
                    {fmtH(w.totalHours)} {t('hr', lang)}{w.totalM2 > 0 ? ` · ${fmt(w.totalM2)} ${t('m2short', lang)}` : ''}
                  </div>
                </div>
                <div style={{ fontSize: 15, fontWeight: 800, color: C.green, fontFamily: F_MONO, letterSpacing: '-0.02em' }}>{fmt(w.final)} {t('uah', lang)}</div>
              </div>
            ))}
          </div>
        </div>
