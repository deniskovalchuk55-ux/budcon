...
import {
  ...
  fmt, fmtH, todayStr,
  saveShift, updateShift, deleteShift, saveShiftsBulk,
...
// Кнопки швидкого внесення суми — при натисканні ПЛЮСУЮТЬСЯ до поточного значення
...
function ShiftForm({ worker, allWorkers, isOwner, onClose, onSaved, tgId, tgName, cfg, lang, cur }) {
  ...
  const actualH = parseFloat(hours) || 0
  const earn = actualH * (sel?.rateHour || 0) + (parseFloat(m2) || 0) * (sel?.rateM2 || 0)

  async function submit(e) {
    e.preventDefault()
    const h = type === 'm2' ? 0 : actualH
    const mm = type === 'hours' ? 0 : (parseFloat(m2) || 0)
    if (!h && !mm) { setError(t('enterHoursOrM2', lang)); return }
    try {
      setLoading(true); setError(null)
      await saveShift({ tgId: sel.tgId, name: sel.name, rateHour: sel.rateHour || 0, rateM2: sel.rateM2 || 0, date, hours: h, m2: mm })
      const detail = `${date}: ${h}${t('hr', lang)} ${mm ? mm + t('m2short', lang) : ''}`
      await writeLog({ tgId: isOwner ? tgId : sel.tgId, name: sel.name, action: 'Shift', details: detail })
      vibrate('success')
      setResult({ hours: h, m2: mm, earned: h * (sel.rateHour || 0) + mm * (sel.rateM2 || 0), date })
      setSuccess(true)
    ...
    <Sheet onClose={onClose} title={t('shiftSaved', lang)}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Card>
          ...
          {result.hours > 0 && <Row label={t('enteredHours', lang)} value={`${fmtH(result.hours)} ${t('hr', lang)}`} color={C.dim} />}
      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <Lbl style={{ margin: 0 }}>{t('amount', lang)}</Lbl>
            {Number(amount) > 0 && <button type="button" onClick={() => setAmount('')} style={{ background: 'none', border: 'none', color: C.red, fontSize: 12, cursor: 'pointer', fontFamily: F, fontWeight: 600 }}>{t('resetAmount', lang)}</button>}
          </div>
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            {QUICK_AMOUNTS.map(a => (
              <button key={a} type="button" onClick={() => addAmount(a)} style={{ flex: 1, padding: '9px 0', borderRadius: 10, border: 'none', background: C.blueSoft, color: C.blue, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: F }}>+{a}</button>
            ))}
          </div>
          <input type="number" inputMode="decimal" value={amount} onChange={e => setAmount(e.target.value)} placeholder={t('orEnterManually', lang)} style={inp} />
        </div>
        {Number(amount) > 0 && (
          <div style={{ background: C.blueSoft, borderRadius: 12, padding: '14px 16px', display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 13, color: C.dim, fontFamily: F }}>{sel?.name}</span>
            <span style={{ fontSize: 20, fontWeight: 800, color: C.blue, fontFamily: F }}>{fmt(Number(amount))} {cur}</span>
          </div>
        )}
        {error && <div style={{ color: C.red, fontSize: 13 }}>{error}</div>}
        <button type="submit" disabled={loading || !amount} style={{ ...primaryBtn, opacity: !amount ? 0.5 : 1 }}>
    ...
    <Sheet onClose={onClose} title={`${t('addBonus', lang)} — ${worker.name}`}>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            {QUICK_AMOUNTS.map(a => (
              <button key={a} type="button" onClick={() => addAmount(a)} style={{ flex: 1, padding: '9px 0', borderRadius: 10, border: 'none', background: C.greenSoft, color: C.green, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: F }}>+{a}</button>
            ))}
          </div>
          <input type="number" inputMode="decimal" value={amount} onChange={e => setAmount(e.target.value)} placeholder={t('orEnterManually', lang)} style={inp} />
        </div>
        {Number(amount) > 0 && (
          <div style={{ background: C.greenSoft, borderRadius: 12, padding: '14px 16px', display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 13, color: C.dim, fontFamily: F }}>{worker.name}</span>
            <span style={{ fontSize: 20, fontWeight: 800, color: C.green, fontFamily: F }}>+{fmt(Number(amount))} {cur}</span>
          </div>
        )}
...
function DebtForm({ worker, mode, onClose, onSaved, tgId, lang, cur }) {
  ...
  const accent = mode === 'issue' ? C.red : C.green
  const accentSoft = mode === 'issue' ? C.redSoft : C.greenSoft
  const addAmount = (a) => { vibrate('light'); setAmount(prev => (Number(prev)||0) + a) }
    ...
    <Sheet onClose={onClose} title={`${mode === 'issue' ? t('issueDebt', lang) : t('payDebt', lang)} — ${worker.name}`}>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            {QUICK_AMOUNTS.map(a => (
              <button key={a} type="button" onClick={() => addAmount(a)} style={{ flex: 1, padding: '9px 0', borderRadius: 10, border: 'none', background: accentSoft, color: accent, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: F }}>+{a}</button>
            ))}
          </div>
          <input type="number" inputMode="decimal" value={amount} onChange={e => setAmount(e.target.value)} placeholder={t('orEnterManually', lang)} style={inp} />
        </div>
        {Number(amount) > 0 && (
          <div style={{ background: accentSoft, borderRadius: 12, padding: '14px 16px', display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 13, color: C.dim, fontFamily: F }}>{worker.name}</span>
            <span style={{ fontSize: 20, fontWeight: 800, color: accent, fontFamily: F }}>{mode === 'issue' ? '+' : '−'}{fmt(Number(amount))} {cur}</span>
          </div>
        )}
...
function EditDayForm({ day, worker, onClose, onSaved, lang }) {
  ...
  async function submit(e) {
    ...
    try {
      setLoading(true); setError(null)
      await updateShift(day.id, { hours: parseFloat(hours) || 0, m2: parseFloat(m2) || 0, date })
      vibrate('success')
      onSaved(); onClose()
    } catch (e) { setError(e.message) } finally { setLoading(false) }
  }
  ...
  return (
    <Sheet onClose={onClose} title={`${t('edit', lang)} — ${date}`}>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div><Lbl>{t('date', lang)}</Lbl><input type="date" value={date} max={todayStr()} onChange={e => setDate(e.target.value)} style={inp} /></div>
        <div><Lbl>{t('hours', lang)}</Lbl><input type="number" inputMode="decimal" step="0.5" value={hours} onChange={e => setHours(e.target.value)} style={inp} /></div>
...
// ════════════════════════════════════════════════════════
// ВІДОМІСТЬ НА ВИПЛАТУ — друкована таблиця
// ════════════════════════════════════════════════════════
function PayslipExportScreen({ allWorkers, year, month, cur, lang, onClose }) {
  const monthLabel = tMonths(lang)[month]
  const [cardAmounts, setCardAmounts] = useState(() => Object.fromEntries(allWorkers.map(w => [w.tgId, 3600])))

  function setCard(tgId, val) {
    setCardAmounts(prev => ({ ...prev, [tgId]: val }))
  }
  function handlePrint() { window.print() }

  const thStyle = { padding: '8px 6px', border: '1px solid #999', fontSize: 11, fontWeight: 700, background: '#f0f0f0', textAlign: 'center' }
  const tdStyle = { padding: '7px 6px', border: '1px solid #bbb', fontSize: 12, textAlign: 'center' }
    ...
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: F }}>
      <style>{`
        @media print {
          ...
          .payslip-input { border: none !important; background: none !important; -webkit-appearance: none; text-align: center; }
        }
      `}</style>

      <div className="no-print" style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, background: C.bg, borderBottom: `1px solid ${C.border}` }}>
        <button onClick={onClose} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, width: 34, height: 34, color: C.text, fontSize: 17, cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>←</button>
        <div style={{ flex: 1, fontSize: 17, fontWeight: 800, color: C.text }}>{t('payslipTitle', lang)}</div>
        <button onClick={handlePrint} style={{ background: C.blue, color: '#fff', border: 'none', borderRadius: 10, padding: '9px 16px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: F }}>🖨️ {t('printBtn', lang)}</button>
      </div>

      <div style={{ padding: 20, color: '#111' }}>
        <div style={{ textAlign: 'center', fontSize: 18, fontWeight: 800, marginBottom: 2 }}>{t('payslipTitle', lang)}</div>
        <div style={{ textAlign: 'center', fontSize: 13, color: '#666', marginBottom: 18 }}>{monthLabel} {year}</div>

        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 680 }}>
            <thead>
              <tr>
                <th style={thStyle}>{t('numLbl', lang)}</th>
                <th style={{ ...thStyle, textAlign: 'left' }}>{t('workerName', lang)}</th>
                <th style={thStyle}>{t('workedTotal', lang)}</th>
                <th style={thStyle}>{t('workDays', lang)}</th>
                <th style={thStyle}>{t('grossTotal', lang)}</th>
                <th style={thStyle}>{t('toCard', lang)}</th>
                <th style={thStyle}>{t('finalCash', lang)}</th>
                <th style={thStyle}>{t('signature', lang)}</th>
              </tr>
            </thead>
            <tbody>
              {allWorkers.map((w, i) => {
                const card = Number(cardAmounts[w.tgId]) || 0
                const final = Math.max(0, w.final - card)
                return (
                  <tr key={w.tgId}>
                    <td style={tdStyle}>{i + 1}</td>
                    <td style={{ ...tdStyle, textAlign: 'left', fontWeight: 600 }}>{w.name}</td>
                    <td style={tdStyle}>{fmtH(w.totalHours)}</td>
                    <td style={tdStyle}>{w.workDays}</td>
                    <td style={tdStyle}>{fmt(w.final)} {cur}</td>
                    <td style={tdStyle}>
                      <input type="number" className="payslip-input" value={cardAmounts[w.tgId]} onChange={e => setCard(w.tgId, e.target.value)}
                        style={{ width: 72, textAlign: 'center', border: '1px solid #ccc', borderRadius: 4, padding: '4px 2px', fontSize: 12, fontFamily: F }} />
                    </td>
                    <td style={{ ...tdStyle, fontWeight: 700 }}>{fmt(final)} {cur}</td>
                    <td style={{ ...tdStyle, minWidth: 90 }}></td>
                  </tr>
                )
              })}
            </tbody>
        ...
        <div style={{ marginTop: 44, display: 'flex', justifyContent: 'space-between', maxWidth: 460 }}>
          <div>
            <div style={{ fontSize: 12, color: '#666', marginBottom: 22 }}>{t('preparedBy', lang)}</div>
            <div style={{ borderBottom: '1px solid #000', width: 200 }} />
          </div>
          <div>
            <div style={{ fontSize: 12, color: '#666', marginBottom: 22 }}>{t('dateLbl', lang)}</div>
            <div style={{ borderBottom: '1px solid #000', width: 120 }} />
          </div>
        </div>
      </div>
    </div>
...
// ════════════════════════════════════════════════════════
function SettingsPage({ cfg, onBack, onSaved, lang, setLang, theme, setTheme, cur, onPrint }) {
  ...
  const [editField, setEditField] = useState(null) // 'rateHour' | 'rateM2' | 'lang' | 'currency'
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, paddingBottom: 40, fontFamily: F }}>
      ...
      <div style={{ padding: '4px 16px 0', display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        {/* Документи */}
        <div>
          <Lbl style={{ marginBottom: 10 }}>{t('printSection', lang)}</Lbl>
          <Card style={{ padding: '4px 12px' }}>
            <div onClick={onPrint} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 0', cursor: 'pointer' }}>
              <IconCircle bg={C.blueSoft} size={34}>🖨️</IconCircle>
              <div style={{ flex: 1, fontSize: 15, color: C.text, fontWeight: 500 }}>{t('printRow', lang)}</div>
              <span style={{ color: C.muted, fontSize: 16 }}>›</span>
            </div>
          </Card>
        </div>
...
// ════════════════════════════════════════════════════════
// ШВИДКЕ МАСОВЕ ВНЕСЕННЯ ГОДИН — для всієї бригади за раз
// ════════════════════════════════════════════════════════
const QUICK_ENTRY_HOURS = [6, 9, 10, 11]

function BulkEntryScreen({ data, allWorkers, cfg, tgId, tgName, lang, cur, year, month, onClose, onSaved }) {
  // Будує список рядків для обраної дати: якщо в цей день у працівника вже є зміна —
  // підтягує її id та години (щоб оновити), інакше — порожній новий рядок.
  function buildEntries(dateStr, defaultH) {
    return allWorkers.map(w => {
      const existing = data.shifts.find(s => s.tgId === w.tgId && s.dateShort === dateStr)
      return {
        tgId: w.tgId, name: w.name, role: w.role, rateHour: w.rateHour, rateM2: w.rateM2,
        id: existing?.id || null,
        hours: existing ? existing.hours : defaultH,
        absent: false, manual: false,
      }
    })
  }
  ...
  const [entries, setEntries] = useState(() => buildEntries(todayStr(), 9))
  ...
  function handleDateChange(newDate) {
    setDate(newDate)
    setEntries(buildEntries(newDate, defaultHours))
  }

  function applyDefault(v) {
    setDefaultHours(v)
    vibrate('light')
    setEntries(prev => prev.map(e => (!e.manual && !e.absent) ? { ...e, hours: v } : e))
  }
  function setHour(idx, v) {
    vibrate('light')
    setEntries(prev => prev.map((e, i) => i === idx ? { ...e, hours: v, manual: true } : e))
  }
  function adjust(idx, delta) {
    setEntries(prev => prev.map((e, i) => i === idx ? { ...e, hours: Math.max(1, Math.min(16, e.hours + delta)), manual: true } : e))
  }
  function toggleAbsent(idx) {
    vibrate('medium')
    setEntries(prev => prev.map((e, i) => i === idx ? { ...e, absent: !e.absent, manual: true } : e))
  }

  const working = entries.filter(e => !e.absent)
  const absent = entries.filter(e => e.absent)
  const updatingCount = working.filter(e => e.id).length
  const totalHours = working.reduce((s, e) => s + e.hours, 0)

  async function confirmSave() {
    setSaving(true)
    try {
      await saveShiftsBulk(working.map(e => ({ id: e.id, tgId: e.tgId, name: e.name, rateHour: e.rateHour, rateM2: e.rateM2, date, hours: e.hours })))
      await writeLog({ tgId, name: tgName, action: 'Bulk shift entry', details: `${date}: ${working.length} ${t('shiftsCount', lang)}, ${absent.length} ${t('bulkSummaryAbsent', lang).toLowerCase()}` })
      await onSaved() // чекаємо поки дані реально підвантажаться з Notion перед закриттям екрана
      vibrate('success')
      setSaved(true)
  ...
  if (saved) return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: F }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: C.text }}>{t('bulkSaved', lang)}</div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, paddingBottom: 40, fontFamily: F }}>
      <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={onClose} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, width: 34, height: 34, color: C.text, fontSize: 17, cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>←</button>
        <div style={{ flex: 1, fontSize: 17, fontWeight: 800, color: C.text }}>{t('bulkEntryTitle', lang)}</div>
        <button onClick={() => setShowConfirm(true)} style={{ background: C.blue, color: '#fff', border: 'none', borderRadius: 10, padding: '9px 16px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: F }}>{t('save', lang)}</button>
      </div>

      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: C.card, border: `1px solid ${C.border2}`, borderRadius: 14, padding: '12px 14px' }}>
          <span style={{ fontSize: 13, color: C.dim, fontWeight: 500 }}>{t('date', lang)}</span>
          <input type="date" value={date} max={todayStr()} onChange={e => handleDateChange(e.target.value)} style={{ border: 'none', background: 'none', fontSize: 15, fontWeight: 700, color: C.text, textAlign: 'right', fontFamily: F }} />
        </div>

        <Card>
          <Lbl>{t('bulkDefaultLbl', lang)}</Lbl>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
            {QUICK_ENTRY_HOURS.map(v => (
              <button key={v} onClick={() => applyDefault(v)} style={{
                padding: '12px 4px', borderRadius: 10, border: 'none', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: F,
                background: defaultHours === v ? C.blue : C.gray, color: defaultHours === v ? '#fff' : C.dim,
              }}>{v}</button>
            ))}
          </div>
          <div style={{ fontSize: 11.5, color: C.muted, marginTop: 10, lineHeight: 1.4 }}>{t('bulkDefaultHint', lang)}</div>
        </Card>

        <Lbl>{t('staff', lang)} ({entries.length})</Lbl>

        {entries.map((e, idx) => (
          <div key={e.tgId} style={{
            background: C.card, borderRadius: 16, padding: 14,
            border: e.absent ? `1.5px solid ${C.red}` : `1.5px solid ${C.border2}`,
            boxShadow: e.absent ? '0 0 0 3px rgba(239,68,68,0.15), 0 0 14px rgba(239,68,68,0.22)' : 'none',
            transition: 'all .2s',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {e.id && !e.absent && <span style={{ fontSize: 10.5, color: C.blue, background: C.blueSoft, padding: '2px 8px', borderRadius: 8, fontWeight: 700 }}>🔄 {t('bulkWillUpdate', lang)}</span>}
                </div>
                {e.role && <div style={{ fontSize: 12, color: C.dim, marginTop: 1 }}>{e.role}</div>}
              </div>
              <button onClick={() => toggleAbsent(idx)} style={{
                border: e.absent ? 'none' : `1px solid ${C.border}`, background: e.absent ? C.red : C.gray,
                color: e.absent ? '#fff' : C.dim, borderRadius: 20, padding: '6px 14px', fontSize: 12.5, fontWeight: 700, cursor: 'pointer', fontFamily: F, flexShrink: 0,
              }}>{e.absent ? `✕ ${t('bulkAbsentBtn', lang)}` : t('bulkAbsentBtn', lang)}</button>
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: 10, opacity: e.absent ? 0.35 : 1, pointerEvents: e.absent ? 'none' : 'auto', transition: 'opacity .2s' }}>
              <div style={{ display: 'flex', gap: 6, flex: 1 }}>
                {QUICK_ENTRY_HOURS.map(v => (
                  <button key={v} onClick={() => setHour(idx, v)} style={{
                    flex: 1, padding: '9px 0', borderRadius: 9, border: e.hours === v ? `1.5px solid ${C.blue}` : 'none',
                    background: e.hours === v ? C.blueSoft : C.gray, color: e.hours === v ? C.blue : C.dim,
                    fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: F,
                  }}>{v}</button>
                ))}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: C.gray, borderRadius: 10, padding: 4, flexShrink: 0 }}>
                <button onClick={() => adjust(idx, -1)} style={{ width: 26, height: 26, borderRadius: 7, border: 'none', background: C.card, color: C.text, fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                <div style={{ fontSize: 14, fontWeight: 800, color: C.text, width: 22, textAlign: 'center' }}>{e.hours}</div>
                <button onClick={() => adjust(idx, 1)} style={{ width: 26, height: 26, borderRadius: 7, border: 'none', background: C.card, color: C.text, fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
              </div>
            </div>
          </div>
      ...
      {showConfirm && (
        <Sheet onClose={() => setShowConfirm(false)} title={`${t('bulkConfirmTitle', lang)} — ${date.split('-').reverse().join('.')}`}>
          <div style={{ display: 'flex', justifyContent: 'space-around', background: C.card, border: `1px solid ${C.border2}`, borderRadius: 14, padding: '14px 16px', marginBottom: 16 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: C.green }}>{working.length}</div>
              <div style={{ fontSize: 11, color: C.dim, marginTop: 2 }}>{t('bulkSummaryWorking', lang)}</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: C.red }}>{absent.length}</div>
              <div style={{ fontSize: 11, color: C.dim, marginTop: 2 }}>{t('bulkSummaryAbsent', lang)}</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: C.text }}>{totalHours}</div>
              <div style={{ fontSize: 11, color: C.dim, marginTop: 2 }}>{t('bulkSummaryHours', lang)}</div>
            </div>
          </div>

          <Lbl>{t('bulkAbsentSection', lang)}</Lbl>
          {absent.length === 0 && <div style={{ color: C.muted, fontSize: 13, padding: '8px 4px 4px' }}>{t('bulkAllPresent', lang)}</div>}
          {absent.map(e => (
            <div key={e.tgId} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 4px', borderBottom: `1px solid ${C.border2}` }}>
              <button onClick={() => setShowConfirm(false)} style={{ width: 30, height: 30, borderRadius: 8, background: C.gray, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, cursor: 'pointer', flexShrink: 0 }}>✏️</button>
              <div style={{ fontSize: 14.5, fontWeight: 700, color: C.text, flex: 1 }}>{e.name}</div>
              <span style={{ fontSize: 12, color: C.red, background: C.redSoft, padding: '4px 10px', borderRadius: 10, fontWeight: 600 }}>{t('bulkAbsentBtn', lang)}</span>
            </div>
          ))}

          <Lbl style={{ marginTop: 14 }}>{t('bulkWorkingSection', lang)}</Lbl>
          {working.map(e => (
            <div key={e.tgId} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 4px', borderBottom: `1px solid ${C.border2}` }}>
              <button onClick={() => setShowConfirm(false)} style={{ width: 30, height: 30, borderRadius: 8, background: C.gray, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, cursor: 'pointer', flexShrink: 0 }}>✏️</button>
              <div style={{ fontSize: 14.5, fontWeight: 700, color: C.text, flex: 1 }}>{e.name}{e.id && <span style={{ fontSize: 10.5, color: C.blue, marginLeft: 6 }}>🔄</span>}</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.blue }}>{e.hours} {t('hr', lang)}</div>
            </div>
          ))}

          <button onClick={confirmSave} disabled={saving} style={{ ...primaryBtn, width: '100%', marginTop: 18, opacity: saving ? 0.7 : 1 }}>
            {saving ? t('saving', lang) : `✓ ${t('bulkConfirmBtn', lang)}`}
          </button>
        </Sheet>
      )}
    </div>
...
// ════════════════════════════════════════════════════════
function ShiftsFeedTab({ data, allWorkers, cfg, year, month, onMonthChange, onMenu, onBulkEntry, lang, setLang, theme, setTheme, cur }) {
  const rows = data.shifts.map(s => ({ ...s, net: s.hours || 0 }))
    .sort((a,b)=>(b.dateShort||'').localeCompare(a.dateShort||''))
  const showBulkBtn = (cfg?.workMode || 'mixed') !== 'm2' && allWorkers.length > 0
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh' }}>
      ...
      {showBulkBtn && (
        <button onClick={onBulkEntry} style={{
          position: 'fixed', right: 16, bottom: 90, background: C.blue, color: '#fff', border: 'none',
          borderRadius: 26, padding: '13px 18px', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8,
          cursor: 'pointer', boxShadow: '0 6px 16px rgba(59,130,246,0.4)', fontFamily: F, zIndex: 15,
        }}>⚡ {t('bulkEntryBtn', lang)}</button>
      )}
    </div>
  )
}
...
function OwnerApp({ data, tgId, tgName, year, month, onMonthChange, onRefresh, lang, setLang, theme, setTheme }) {
  ...
  if (showSettings) return (
    <SettingsPage cfg={data.cfg} onBack={() => setShowSettings(false)} onSaved={onRefresh} lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} cur={cur} onPrint={() => { setShowSettings(false); setShowPayslip(true) }} />
  )

  if (showPayslip) return (
    <PayslipExportScreen allWorkers={allWorkers} year={year} month={month} cur={cur} lang={lang} onClose={() => setShowPayslip(false)} />
  )

  if (showBulkEntry) return (
    <BulkEntryScreen
      data={data} allWorkers={allWorkers} cfg={data.cfg} tgId={tgId} tgName={tgName}
      lang={lang} cur={cur} year={year} month={month}
      onClose={() => setShowBulkEntry(false)} onSaved={onRefresh}
    />
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg }}>
      ...
      {navTab === 'shifts' && <ShiftsFeedTab data={data} allWorkers={allWorkers} cfg={data.cfg}
        year={year} month={month} onMonthChange={onMonthChange} onMenu={() => setShowSettings(true)} onBulkEntry={() => setShowBulkEntry(true)} lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} cur={cur} />}
...
export default function App() {
  ...
  function onMonthChange(y, m) { setYear(y); setMonth(m); return load(y, m) }
  function onRefresh() { return load(year, month) }
