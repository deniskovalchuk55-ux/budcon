...
function dateFilter(field, year, month) {
  ...
  return { and: [
    ...
    { property: field, date: { before: nextStart } },
  ]}
}
...
export async function saveShift({ tgId, name, rateHour, rateM2, date, hours, m2 }) {
  return createPage(DB.shifts, {
    ...
    'LunchBreak': { number: 0 }, // обід оплачуваний, не віднімається — лишаємо 0 для сумісності зі старими записами
    'RateHour': { number: Number(rateHour)||0 },
    'RateM2': { number: Number(rateM2)||0 },
  })
}

export async function updateShift(pageId, { hours, m2, date }) {
  const props = {}
...
// Масове внесення змін за раз (швидке внесення для всієї бригади).
// entries: [{ id?, tgId, name, rateHour, rateM2, date, hours }]
// Якщо передано id (запис на цю дату вже існує) — оновлює його, інакше створює новий.
// Виконується послідовно (Notion API не має bulk-create/update), але швидко для типової бригади (5-30 записів).
export async function saveShiftsBulk(entries) {
  const results = []
  for (const e of entries) {
    const r = e.id
      ? await updateShift(e.id, { hours: e.hours })
      : await saveShift({ tgId: e.tgId, name: e.name, rateHour: e.rateHour, rateM2: e.rateM2, date: e.date, hours: e.hours, m2: 0 })
    results.push(r)
  }
  return results
}
...
export function calcSalary(tgId, data) {
  ...
  const days = myShifts.map(s => {
    const net = s.hours || 0 // обід оплачуваний — не віднімається
    totalHours += net
    totalM2 += s.m2||0
