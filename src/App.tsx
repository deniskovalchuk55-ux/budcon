      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {type !== 'hours' && (
          <div><Lbl>{t('m2', lang)}</Lbl>
            ...
            {m2 && (sel?.rateM2 || 0) > 0 && <div style={{ fontSize: 12, color: C.text, marginTop: 6, fontFamily: F }}>= {fmt(parseFloat(m2) * (sel?.rateM2 || 0))} {t('uah', lang)}</div>}
          </div>
        )}
        ...
        {earn > 0 && (
          <div style={{ background: C.blueSoft, borderRadius: 12, padding: '14px 16px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontSize: 20, fontWeight: 800, color: C.blue, fontFamily: F }}>{fmt(earn)} {t('uah', lang)}</span>
          </div>
        )}
...
function AdvanceForm({ worker, allWorkers, isOwner, onClose, onSaved, tgId, tgName, lang }) {
  ...
  if (success) return (
    <Sheet onClose={onClose} title={t('advanceSaved', lang)}>
      <div style={{ textAlign: 'center', padding: 20, fontSize: 28, fontWeight: 800, color: C.blue, fontFamily: F }}>{fmt(amount)} {t('uah', lang)}</div>
    </Sheet>
  )
...
function BonusForm({ worker, onClose, onSaved, tgId, lang }) {
  ...
  if (success) return (
    <Sheet onClose={onClose} title={t('bonusSaved', lang)}>
      <div style={{ textAlign: 'center', padding: 20, fontSize: 28, fontWeight: 800, color: C.green, fontFamily: F }}>+{fmt(amount)} {t('uah', lang)}</div>
    </Sheet>
  )
...
function DebtForm({ worker, mode, onClose, onSaved, tgId, lang }) {
  ...
  if (success) return (
    <Sheet onClose={onClose} title={mode === 'issue' ? t('debtSaved', lang) : t('debtPaymentSaved', lang)}>
      <div style={{ textAlign: 'center', padding: 20, fontSize: 28, fontWeight: 800, color: mode === 'issue' ? C.red : C.green, fontFamily: F }}>{mode === 'issue' ? '+' : '−'}{fmt(amount)} {t('uah', lang)}</div>
    </Sheet>
  )
...
// ════════════════════════════════════════════════════════
function SettingsPage({ cfg, onBack, onSaved, lang, setLang, theme, setTheme }) {
  ...
  const [editField, setEditField] = useState(null) // 'rateHour' | 'rateM2' | 'lunch' | 'lang'
        ...
        <div>
          ...
          <Card style={{ padding: '4px 12px' }}>
            <div onClick={() => setEditField('rateHour')} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 0', borderBottom: `1px solid ${C.border2}`, cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: 14, color: C.dim }}>{fmt(defaultRateHour)} ₴</div>
            ...
            <div onClick={() => setEditField('rateM2')} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 0', cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: 14, color: C.dim }}>{fmt(defaultRateM2)} ₴</div>
              <span style={{ color: C.muted, fontSize: 16 }}>›</span>
            </div>
          </Card>
        </div>
...
function WorkerRow({ w, onClick }) {
  return (
    <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 4px', cursor: 'pointer', borderBottom: `1px solid ${C.border2}` }}>
      ...
      <div style={{ fontSize: 15, fontWeight: 700, color: C.blue, fontFamily: F, whiteSpace: 'nowrap' }}>{fmt(w.final)} ₴</div>
      <span style={{ color: C.muted, fontSize: 16 }}>›</span>
    </div>
  )
}
...
// ════════════════════════════════════════════════════════
function ShiftRow({ day, rateHour, rateM2, showHours, showM2, workerName, onEdit, lang }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 12, padding: '13px 4px', borderBottom: `1px solid ${C.border2}`, alignItems: 'flex-start' }}>
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }} onClick={() => onEdit?.(day)}>
        <div style={{ fontSize: 15, fontWeight: 800, color: C.blue, fontFamily: F, cursor: onEdit ? 'pointer' : 'default' }}>{fmt(earned)} ₴</div>
...
function ShiftsList({ days, rateHour, rateM2, cfg, onEdit, lang, showWorkerNames, allWorkers }) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 6px' }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 800, color: C.text, fontFamily: F }}>{fmt(totalEarn)} ₴</div>
      </div>
    </>
  )
}
...
function WorkerDetail({ w, allWorkers, data, onBack, onRefresh, tgId, tgName, cfg, lang, year, month, onMonthChange }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, paddingBottom: 40, fontFamily: F }}>
      ...
      <div style={{ padding: '16px 16px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {tab === 'shifts' && <>
          <HeroCard label={`${t('toPayForMonth', lang)} ${monthLabel}`} amount={`${fmt(w.final)} ₴`}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginTop: 16, position: 'relative' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: '#fff', fontWeight: 700, fontFamily: F, marginTop: 2 }}>{fmt(w.earnHours)} ₴</div>
              </div>
              <div>
                <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.7)', fontFamily: F }}>{t('byM2Lbl', lang)}</div>
                <div style={{ fontSize: 13, color: '#fff', fontWeight: 700, fontFamily: F, marginTop: 2 }}>{fmt(w.earnM2)} ₴</div>
              </div>
              <div>
          ...
          <ShiftsList days={w.days} rateHour={w.rateHour} rateM2={w.rateM2} cfg={cfg} onEdit={setEditDay} lang={lang} />
        </>}
        ...
        {tab === 'profile' && (
          <Card style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <Row label={t('rateHourRow', lang)} value={`${fmt(w.rateHour)} ₴`} />
            <Row label={t('rateM2Row', lang)} value={`${fmt(w.rateM2)} ₴`} last />
          </Card>
        )}
        ...
        {tab === 'accruals' && (
          <Card>
            ...
            <Row label={t('byHoursLbl', lang)} value={`${fmt(w.earnHours)} ₴`} />
            <Row label={t('byM2Lbl', lang)} value={`${fmt(w.earnM2)} ₴`} />
            {w.manualBonus > 0 && <Row label={t('manualBonusLbl', lang)} value={`+${fmt(w.manualBonus)} ₴`} color={C.green} />}
            <Row label={t('gross', lang)} value={`${fmt(w.gross)} ₴`} bold />
            {w.totalAdv > 0 && <Row label={t('advancesTotal', lang)} value={`−${fmt(w.totalAdv)} ₴`} color={C.red} />}
            {w.debtPaidMonth > 0 && <Row label={t('debtPaid', lang)} value={`−${fmt(w.debtPaidMonth)} ₴`} color={C.red} />}
            <Row label={t('toPay', lang)} value={`${fmt(w.final)} ₴`} color={C.blue} bold last />
            {w.debtRemaining > 0 && (
              <div style={{ marginTop: 12, fontSize: 13, color: C.red, background: C.redSoft, borderRadius: 10, padding: '10px 12px' }}>
                {t('debtRemaining', lang)}: {fmt(w.debtRemaining)} ₴
              </div>
            )}
          </Card>
        )}
            ...
            {history.map(h => {
              ...
              return (
                <div key={h.kind+h.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: `1px solid ${C.border2}` }}>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 700, color: cfgByKind.color }}>{cfgByKind.sign}{fmt(h.amount)} ₴</div>
                </div>
              )
            })}
          </Card>
...
// ════════════════════════════════════════════════════════
function HomeTab({ data, allWorkers, totalFinal, onOpenWorker, onMenu, onAddStaff, lang, setLang, theme, setTheme }) {
  return (
    <div style={{ padding: '0 16px 90px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <HeroCard label={t('totalToPayMonth', lang)} amount={`${fmt(totalFinal)} ₴`} />
      ...
      <Card style={{ padding: '0 12px' }}>
        ...
        {allWorkers.map(w => <WorkerRow key={w.tgId} w={w} onClick={() => onOpenWorker(w.tgId)} />)}
      </Card>
    </div>
  )
}
...
// ════════════════════════════════════════════════════════
function TeamTab({ allWorkers, onOpenWorker, onMenu, onAddStaff, lang, setLang, theme, setTheme }) {
  return (
    <div style={{ padding: '0 16px 90px', display: 'flex', flexDirection: 'column', gap: 16 }}>
...
// ════════════════════════════════════════════════════════
function ShiftsFeedTab({ data, allWorkers, cfg, year, month, onMonthChange, onMenu, lang, setLang, theme, setTheme }) {
        ...
        <Card style={{ padding: '0 12px' }}>
          {rows.map(d => {
            ...
            return <ShiftRow key={d.id} day={d} rateHour={d.rateHour} rateM2={d.rateM2}
              showHours={cfg?.showHours ?? true} showM2={cfg?.showM2 ?? true}
              workerName={w?.name || d.name} lang={lang} />
          })}
        </Card>
      )}
    </div>
...
// ════════════════════════════════════════════════════════
function ReportTab({ allWorkers, totalFinal, year, month, onMonthChange, onMenu, lang, setLang, theme, setTheme }) {
  ...
  return (
    <div style={{ padding: '0 16px 90px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <Card>
        ...
        <Row label={t('reportTotalGross', lang)} value={`${fmt(totalGross)} ₴`} />
        <Row label={t('reportTotalPaid', lang)} value={`${fmt(totalFinal)} ₴`} color={C.blue} bold />
        <Row label={t('reportTotalDebt', lang)} value={totalDebt>0 ? `${fmt(totalDebt)} ₴` : '—'} color={totalDebt>0?C.red:C.dim} last />
      </Card>

      <Lbl>{t('reportByWorker', lang)}</Lbl>
      <Card style={{ padding: '0 12px' }}>
        {allWorkers.map(w => (
          <Row key={w.tgId} label={w.name} value={`${fmt(w.final)} ₴`} />
        ))}
      </Card>
    </div>
  )
...
// ════════════════════════════════════════════════════════
function WorkerHomeTab({ w, cfg, tgId, tgName, year, month, onMonthChange, onRefresh, lang }) {
  const [modal, setModal] = useState(null)
  const [editDay, setEditDay] = useState(null)
  const monthLabel = tMonths(lang)[month].toLowerCase()
  ...
  return (
    <div style={{ padding: '0 16px 90px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <HeroCard label={`${t('toPayForMonth', lang)} ${monthLabel}`} amount={`${fmt(w.final)} ₴`}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginTop: 16, position: 'relative' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: '#fff', fontWeight: 700, marginTop: 2 }}>{fmt(w.earnHours)} ₴</div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.7)' }}>{t('byM2Lbl', lang)}</div>
            <div style={{ fontSize: 13, color: '#fff', fontWeight: 700, marginTop: 2 }}>{fmt(w.earnM2)} ₴</div>
          </div>
        </div>
      </HeroCard>
      ...
      {modal === 'shift' && <ShiftForm worker={w} allWorkers={[w]} isOwner={false} onClose={() => setModal(null)} onSaved={onRefresh} tgId={tgId} tgName={tgName} cfg={cfg} lang={lang} />}
...
function WorkerReportTab({ w, year, month, onMonthChange, lang }) {
  const monthLabel = tMonths(lang)[month]
  return (
    <div style={{ padding: '0 16px 90px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ padding: '14px 0 0', textAlign: 'center' }}>
...
function OwnerApp({ data, tgId, tgName, year, month, onMonthChange, onRefresh, lang, setLang, theme, setTheme }) {
  ...
  if (showSettings) return (
    <SettingsPage cfg={data.cfg} onBack={() => setShowSettings(false)} onSaved={onRefresh} lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} />
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg }}>
      {navTab === 'home' && <HomeTab data={data} allWorkers={allWorkers} totalFinal={totalFinal}
        onOpenWorker={setSelWorkerId} onMenu={() => setShowSettings(true)} onAddStaff={() => setShowAddStaff(true)} lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} />}
      {navTab === 'shifts' && <ShiftsFeedTab data={data} allWorkers={allWorkers} cfg={data.cfg}
        year={year} month={month} onMonthChange={onMonthChange} onMenu={() => setShowSettings(true)} lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} />}
      {navTab === 'team' && <TeamTab allWorkers={allWorkers} onOpenWorker={setSelWorkerId}
        onMenu={() => setShowSettings(true)} onAddStaff={() => setShowAddStaff(true)} lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} />}
      ...
      {showAddShift && <ShiftForm worker={allWorkers[0]} allWorkers={allWorkers} isOwner={true}
        onClose={() => setShowAddShift(false)} onSaved={onRefresh} tgId={tgId} tgName={tgName} cfg={data.cfg} lang={lang} />}
    </div>
  )
}
...
function WorkerApp({ data, tgId, tgName, year, month, onMonthChange, onRefresh, lang, setLang, theme, setTheme }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg }}>
      ...
      {navTab === 'home' && <WorkerHomeTab w={w} cfg={data.cfg} tgId={tgId} tgName={tgName} year={year} month={month} onMonthChange={onMonthChange} onRefresh={onRefresh} lang={lang} />}
      {navTab === 'report' && <WorkerReportTab w={w} year={year} month={month} onMonthChange={onMonthChange} lang={lang} />}
